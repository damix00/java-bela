...
export default function GameView() {
    ...
    const { team1Score, team2Score } = useMemo(() => {
        if (!game || !bottomPlayer) return { team1Score: 0, team2Score: 0 };

        const activeRound = game.currentRound;
        const includeRoundPoints =
            activeRound !== null &&
            activeRound.roundStatus !== RoundStatus.FINISHED;
        const team1Score =
        ...
        const myTeamIndex = bottomPlayer.seatIndex % 2;
    ...
    const isPlayingCards =
        game?.currentRound?.roundStatus === RoundStatus.PLAYING;
    ...
    const isDraggingCard =
        tableState.trickKey === tableStateKey
            ? tableState.isDraggingCard
            : false;
    const activePreviewCard =
        ...
        playedCards.some(
            ...
            ? null
            : previewCard;
    const previewCardKey = activePreviewCard
        ? getCardKey(activePreviewCard)
        : null;
        ...
        (card: Card, _index: number, source: "click" | "drag") => {
            if (!legalMoveCardKeys.has(getCardKey(card))) {
                toast.error("Invalid move", {
                    description:
                        "You need to play a legal card for this trick.",
        ...
        (dragging: boolean) => {
            setTableState((current) => {
                const nextPreviewCard =
                    current.trickKey === tableStateKey
                        ? current.previewCard
                        : null;
                const nextState = {
                    trickKey: tableStateKey,
                    ...
                    {bottomPlayer &&
                        ...
                        turnTimer && (
                            <TurnTimeout
                                ...
                                label={
                                    turnTimer.currentTurnIndex ===
                                    bottomPlayer.seatIndex
                                        ? "Throw your card"
                    ...
                    <div className="absolute left-1/2 top-0 z-10 -translate-x-1/2 md:top-4">
                        {topPlayer && (
                            <PlayerSeat
                                ...
                                position="top"
                                isCurrentTurn={
                                    topPlayer.seatIndex === currentTurnSeatIndex
                                }
                            />
                        )}
                    </div>
