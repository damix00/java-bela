 ...
 *
 * A route handler rather than a server action because it must be callable from
 * arbitrary client code on a 401 — from `apiFetch` and, later, from the
 * WebSocket connect path, neither of which runs inside a React event or
 * transition.
 */
export async function POST(request: NextRequest) {
    ...
    if (origin && origin !== request.nextUrl.origin) {
        return NextResponse.json(
            {
                code: "BAD_ORIGIN",
                message: "Cross-origin refresh is not allowed",
            },
            { status: 403 },
        );
    }
    ...
    // The backend is unreachable or broken. Leave the cookies alone — a transient
    // outage must not end a perfectly good session.
    return NextResponse.json(
