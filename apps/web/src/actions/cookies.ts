...
// The names live in an import-free module so the edge-side proxy can read them
// without pulling this file's `server-only` dependency in with them.
export {
  ACCESS_TOKEN_COOKIE,
 ...
 * from NextResponse, so route handlers and server actions share one writer.
 */
type CookieWriter = {
